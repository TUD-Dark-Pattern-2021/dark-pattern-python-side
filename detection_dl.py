...
#    ocr = ocr()
    ...
    # -------------------------- Checking Presence ------------------
     ...
     # Loading the saved Tokenizer with joblib, CNN model with keras
    presence_model = keras.models.load_model('CNN_model1.h5')
    presence_cv = joblib.load("Presence_Tokenizer.joblib")

    # New dataset to predict
    presence_pred = pd.DataFrame(j_data)

    # filter type == text, OCR
...
#                '% off', 'in stock soon', 'problem', 'UTC', 'javascript', 'cookie', 'cookies', 'disclaimer','https']
    ...
    # ---- Text Vectorization
    # Vectorization
    content = presence_cv.texts_to_sequences(presence_pred['content'])
    ...
    # ---- Make Prediction
    pred = presence_model.predict_classes(pad_content)
    pred_list = pred.flatten()

    presence_pred['presence'] = pred_list.tolist()
